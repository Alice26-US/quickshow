...
const Footer = () => {
  ...
  const footerNavClass = ({ isActive }) =>
    `transition-colors ${isActive ? "text-blue-400 underline underline-offset-4" : "text-gray-300 hover:text-white"}`
  ...
  return (
    <footer className="px-6 md:px-16 lg:px-36 mt-40 w-full text-gray-300">
      <div className="flex flex-col md:flex-row justify-between w-full gap-10 border-b border-gray-500 pb-14">
        <div className="md:max-w-96">
          ...
          <p className="mt-6 text-sm text-gray-400">
            Built with love in Douala, Cameroon. We are dedicated to providing accessible, high-quality,
            and AI-enabled educational tools to empower students across the nation and beyond.
          </p>
        ...
        <div className="flex-1 flex items-start md:justify-end gap-20 md:gap-40">
          <div>
            ...
            <ul className="text-sm space-y-2">
              {quickLinks.map((link) => (
                <li key={link.label}>
                  <NavLink to={link.to} end={link.end} className={footerNavClass}>
                    {link.label}
                  </NavLink>
                </li>
              ))}
      ...
      <p className="pt-4 text-center text-sm pb-5 text-gray-500">
        Copyright {new Date().getFullYear()} (c) QuickLearn Cameroon. All Rights Reserved.
      </p>
    </footer>
  )
}
