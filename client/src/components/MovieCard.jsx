...
const MovieCard = ({movie}) => {
    ...
    return (
        <div className="flex flex-col justify-between p-3 bg-gray-300 rounded-2xl hover:-transition duration-300 w-66">
            <img onClick={()=> {navigate(`/movies/${movie._id}`); scrollTo(0,0)}}
             ...
             <p className="font-semibold mt-2 truncate">{movie.tittle}</p>

             <p className="text-sm text-gray-400 mt-y">
                {new Date(movie.release_date).getFullYear()} . {movie.
                 genres.slice(0,2).map(genre => genre.name). join(" | ")} . 
                {timeFormat(movie.runtime)}
             </p>

             <div className="flex items justify-between mt-4 pb-3">
