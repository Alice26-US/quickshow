...
const ListTopics = () => {
    ...
    const handleDeleteTopic = async (topicId, topicTitle) => {
        const confirmed = window.confirm(`Delete "${topicTitle}" and its uploaded files?`);
        if (!confirmed) return;

        try {
            const { data } = await axios.delete(`${API_URL}/topics/${topicId}`);
            if (!data.success) {
                        ...
                        <tbody className="divide-y divide-gray-800/50">
                            {loading ? (
                                ...
                                filteredTopics.map((topic) => (
                                    <tr key={topic._id} className="hover:bg-gray-800/40 transition-colors group">
                                        <td className="px-6 py-4">
                                            <div className="font-medium text-gray-200">{topic.title}</div>
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={`px-2.5 py-1 rounded-full text-xs font-medium ${getLevelBadge(topic.level)}`}>
                                                {topic.level}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4 text-center font-mono">{topic.videos?.length || 0}</td>
                                        <td className="px-6 py-4 text-center font-mono">{topic.flashcards?.length || 0}</td>
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <Link to={`/topics/${topic._id}`} className="p-1.5 text-gray-400 hover:text-blue-400 hover:bg-blue-400/10 rounded transition-colors tooltip" title="Preview Topic">
                                                    <ExternalLink size={16} />
                                                </Link>
                                                <Link to={`/admin/edit-topic/${topic._id}`} className="p-1.5 text-gray-400 hover:text-emerald-400 hover:bg-emerald-400/10 rounded transition-colors tooltip" title="Edit Topic">
                                                    <Edit size={16} />
                                                </Link>
                                                <button
                                                    className="p-1.5 text-gray-400 hover:text-rose-400 hover:bg-rose-400/10 rounded transition-colors tooltip"
                                                    title="Delete Topic"
